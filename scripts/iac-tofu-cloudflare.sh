...
if [[ "$#" -eq 0 ]]; then
  echo "Usage: scripts/iac-tofu-cloudflare.sh <tofu command args>" >&2
  echo "Example: scripts/iac-tofu-cloudflare.sh plan" >&2
  exit 1
fi
...
if [[ -z "${cloudflare_api_token}" ]]; then
  echo "Cloudflare API token was not found in OpenBao." >&2
  exit 1
fi
...
tofu -chdir="${repo_root}/terraform/cloudflare" "$@"
